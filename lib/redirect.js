...
function verify(startUrl, configuredEndUrl, cb) {
    ...
    if (response.statusCode === 200) {
      ...
      console.log('Couldnt fetch the url:' + startUrl + ' statusCode:' + response.statusCode);
        ...
        nonWorkingStartUrls.push({'startUrl': startUrl, 'endUrl':configuredEndUrl,  'code':response.statusCode, 'error':'The end url ' + response.request.uri.href + ' gets an ' + response.statusCode + ' error'});

        console.log('Couldnt fetch the url (' + startUrl + '):' + response.request.uri.href + ' statusCode:' + response.statusCode);
      }
    }
  }
    cb();
